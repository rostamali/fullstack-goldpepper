...
const FileView: FC<FileCardType> = ({ file, heightClass }) => {
	return (
		<div className="file-view rounded-md overflow-hidden relative group border border-gray-muted border-opacity-20">
			{file.fileType === 'image' && (
				<Image
			...
			{/* {file.fileType === 'video' && (
				<VideoPlayer url={file.url} className={'h-[220px] w-full'} />
			)}
			{file.fileType !== 'video' && (
